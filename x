        ...
        if BodyPart then
            ...
            if getgenv().Silent.Startup.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if getgenv().Silent.Startup.mode == "target" then
                    ...
                    -- Check if the targeting guard prevents shooting based on the set conditions
                    if getgenv().TargetingGuard.BlockGroundShots and IsTargetingGround(TargetPlayer.Character) then
                        print("Targeting the ground is blocked")
                        return -- Prevent shooting at the ground
                    end
                    
                    -- Apply TargetLockout if enabled
                    if getgenv().TargetingGuard.TargetLockout.Active then
                        local distance = CalculateTargetDistance(TargetPlayer.Character)  -- You need to define a function to calculate distance
                        if getgenv().TargetingGuard.TargetLockout.Method == "PixelCheck" then
                            if distance.Horizontal > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Horizontal or 
                               distance.Vertical > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Vertical then
                                print("Target lockout due to distance check")
                                return -- Lockout if target is too far
                            end
                        elseif getgenv().TargetingGuard.TargetLockout.Method == "AngleRestriction" then
                            local angle = CalculateTargetAngle(TargetPlayer.Character)  -- Define this function to get the angle
                            if angle > getgenv().TargetingGuard.TargetLockout.AngleThreshold.MaxAngle then
                                print("Target lockout due to angle restriction")
                                return -- Lockout if target angle exceeds the threshold
                            end
                        end
                    end
                    ...
                    -- Apply YAxisAdjustment if enabled
                    if getgenv().TargetingGuard.YAxisAdjustment == "strict" then
                        closestPoint = ApplyStrictYAxisAdjustment(closestPoint)  -- Define this function for strict adjustments
                    elseif getgenv().TargetingGuard.YAxisAdjustment == "moderate" then
                        closestPoint = ApplyModerateYAxisAdjustment(closestPoint)  -- Define this function for moderate adjustments
                    elseif getgenv().TargetingGuard.YAxisAdjustment == "relaxed" then
                        closestPoint = ApplyRelaxedYAxisAdjustment(closestPoint)  -- Define this function for relaxed adjustments
                    end
                ...
                elseif getgenv().Silent.Startup.mode == "normal" then
                    ...
                    if targetToShoot and targetToShoot.Character then
                        ...
                        -- Check if the targeting guard prevents shooting based on the set conditions
                        if getgenv().TargetingGuard.BlockGroundShots and IsTargetingGround(targetToShoot.Character) then
                            print("Targeting the ground is blocked")
                            return -- Prevent shooting at the ground
                        end
                        ...
                        -- Apply TargetLockout if enabled
                        if getgenv().TargetingGuard.TargetLockout.Active then
                            local distance = CalculateTargetDistance(targetToShoot.Character)  -- You need to define a function to calculate distance
                            if getgenv().TargetingGuard.TargetLockout.Method == "PixelCheck" then
                                if distance.Horizontal > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Horizontal or 
                                   distance.Vertical > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Vertical then
                                    print("Target lockout due to distance check")
                            ...
                            elseif getgenv().TargetingGuard.TargetLockout.Method == "AngleRestriction" then
                                local angle = CalculateTargetAngle(targetToShoot.Character)  -- Define this function to get the angle
                                if angle > getgenv().TargetingGuard.TargetLockout.AngleThreshold.MaxAngle then
                                    print("Target lockout due to angle restriction")
                                    return -- Lockout if target angle exceeds the threshold
                                end
...
local function isPartVisible(part)
    -- Check if Silent mode allows wall check
    if not getgenv().Silent.Startup.WallCheck then
        return true
    end
    
    -- Prevent targeting the ground if BlockGroundShots is enabled in the TargetingGuard
    if getgenv().TargetingGuard.BlockGroundShots and IsTargetingGround(part) then
        print("Part is on the ground and targeting is blocked")
        return false -- Prevent targeting parts on the ground
    end
    ...
    -- Perform the raycast to check visibility
    local hit = Workspace:FindPartOnRayWithIgnoreList(ray, {Local_Player.Character, part.Parent})

    -- If targeting lockout is enabled, we should also check the distance or angle to the target part
    if getgenv().TargetingGuard.TargetLockout.Active then
        local distance = (origin - part.Position).Magnitude
        if getgenv().TargetingGuard.TargetLockout.Method == "PixelCheck" then
            local horizontalDist = math.abs(part.Position.X - origin.X)
            local verticalDist = math.abs(part.Position.Y - origin.Y)
            if horizontalDist > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Horizontal or
               verticalDist > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Vertical then
                print("Target lockout due to distance check")
                return false -- Don't consider the part if it's out of lockout distance
            end
        elseif getgenv().TargetingGuard.TargetLockout.Method == "AngleRestriction" then
            local angle = CalculateTargetAngle(part) -- You need to define this function based on angle calculation
            if angle > getgenv().TargetingGuard.TargetLockout.AngleThreshold.MaxAngle then
                print("Target lockout due to angle restriction")
                return false -- Don't consider the part if the angle is too high
            end
        end
    end

    -- If a hit was detected, the part is visible, otherwise it's not
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if humanoid then
            ...
            -- Apply resolver logic if enabled
            ...
            elseif velocity.Magnitude > 50 and getgenv().Silent.Startup.Resolver then
                ...
                -- Reset prediction if no resolver is active
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Silent.Startup.Enabled and IsTargeting then
        UpdateFOV()  -- Refresh FOV visibility
        
        -- Ensure TargetingGuard restrictions are respected
        local function checkTargetLockout(targetPlayer)
            local targetPos = targetPlayer.Character.Head.Position
            if getgenv().TargetingGuard.TargetLockout.Active then
                local distance = (Current_Camera.CFrame.Position - targetPos).Magnitude
    
                -- Distance check based on the TargetLockout method
                if getgenv().TargetingGuard.TargetLockout.Method == "PixelCheck" then
                    local horizontalDist = math.abs(targetPos.X - Current_Camera.CFrame.Position.X)
                    local verticalDist = math.abs(targetPos.Y - Current_Camera.CFrame.Position.Y)
                    if horizontalDist > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Horizontal or
                       verticalDist > getgenv().TargetingGuard.TargetLockout.DistanceLimits.Vertical then
                        return false -- Target is out of range
                    end
                elseif getgenv().TargetingGuard.TargetLockout.Method == "AngleRestriction" then
                    local angle = CalculateTargetAngle(targetPlayer)  -- Implement this function to compute angle
                    if angle > getgenv().TargetingGuard.TargetLockout.AngleThreshold.MaxAngle then
                        return false -- Target is too far away by angle
                    end
                end
            end
        ...
        if getgenv().Silent.Startup.mode == "target" and TargetPlayer then
            if TargetPlayer.Character then
                ...
                -- Health check: Skip if health is under 7
                ...
                -- Check visibility: Skip if behind a wall
                ...
                -- Check if target is valid according to TargetingGuard
                if not checkTargetLockout(TargetPlayer) then
                    IsTargeting = false
                    LastTarget = TargetPlayer
                    return
                end
        ...
        elseif getgenv().Silent.Startup.mode == "normal" then
            ...
            if target and target.Character then
                ...
                -- Check if target is valid according to TargetingGuard
                if not checkTargetLockout(target) then
                    return
                end
    ...
    elseif LastTarget and LastTarget.Character then
        -- Re-target if last target reappears and is visible
        ...
        Fov.Visible = false  -- Hide FOV if not targeting
    end
